...
const writeOutput = async output => {
  const outputFile = path.resolve(process.cwd(), 'sar-mentors-output.log');
  await fs.writeFile(outputFile, output);

  log.info('Printed output to'.blue, `${outputFile}`);
};

async function main(colleagues, { sorting: sortArg, office: officeArg } = {}) {
  ...
  log('...');
  log(
    `=> ${result.others.length} in ${mainOffice} has mentors in other offices`,
  );
